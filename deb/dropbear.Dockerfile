...
RUN for f in debian/dropbear.postinst debian/dropbear-initramfs.postinst debian/initramfs/dropbear-hook; do \
      sed -i -e 's/for keytype in [^;]\+/for keytype in dss rsa ecdsa ed25519/' $f && \
      sed -i -e 's/(dss|rsa)/(dss|rsa|ed25519)/' $f; \
    done
...
# Maybe set ip= parameter in grub config for klibc ipconfig if not using DHCP.
# update-initramfs -k all -u
#
# === Unlock LUKS (cryptsetup-initramfs) ===
# cryptroot-unlock
#
# === Unlock ZFS native encryption (zfs-initramfs) ===
# /sbin/zfs load-key rpool && killall zfs    # kill 'zfs load-key' on console
